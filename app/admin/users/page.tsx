...
interface User {
  ...
  name: string
  email: string
  role: "admin" | "mentor" | "mentee" | "admin+mentor"
...
export default function AdminUsers() {
  ...
  // This useEffect is no longer needed since we only create mentors
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        mentorsSnapshot.forEach((doc) => {
          const data = doc.data()
          const user = { uid: doc.id, ...data } as User
          usersArray.push(user)
          mentorsArray.push(user)
        })

        // Fetch mentees from mentees collection
        const menteesSnapshot = await getDocs(collection(db, "mentees"))
        menteesSnapshot.forEach((doc) => {
          const data = doc.data()
          const user = { uid: doc.id, ...data } as User
  ...
  const handleSaveMenteeChanges = async (menteeId: string) => {
    ...
    try {
      ...
      await updateDoc(menteeRef, updateData)
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      // Find which collection the user belongs to and delete from appropriate collection
      ...
      } else if (user.role === "admin" || user.role === "admin+mentor") {
  ...
  const handleCreateMentor = async () => {
    ...
    try {
      ...
      // Save user data in mentors collection since we only create mentors
      const collectionName = "mentors"

      await setDoc(doc(db, collectionName, mentorUser.uid), userData)

      setCreationStatus("Securing session...");
      // Sign out the newly created account (read-only security process)
      await signOut(auth)
      console.log(`âœ“ Auto-signed out new user (security process)`);

      // Sign back in as admin
      if (adminEmail) {
        try {
          await signInWithEmailAndPassword(auth, adminEmail, adminPassword)
          console.log(`âœ“ Signed back in as admin`);
        } catch (error) {
          console.error("âŒ Error signing back in as admin:", error)
        }
      }
    ...
    try {
      ...
      for (let i = 0; i < bulkMentors.length; i++) {
        ...
        try {
          ...
          const mentorUser = mentorCredential.user
          console.log(`âœ“ Created auth account for ${mentorData.name}`);
          ...
          // Save in mentors collection since role is mentor
          await setDoc(doc(db, "mentors", mentorUser.uid), userData)
          console.log(`âœ“ Saved user data for ${mentorData.name} with mentor ID: ${mentorId}`);
          ...
          try {
            ...
            await new Promise((resolve) => {
              signOut(auth).then(() => {
                console.log(`âœ“ Auto-signed out ${mentorData.name} (security process)`);
                resolve(true);
              }).catch((signOutError) => {
          ...
          if (adminEmail && i < bulkMentors.length - 1) {
            try {
              await signInWithEmailAndPassword(auth, adminEmail, adminPassword)
              console.log(`âœ“ Signed back in as admin for next iteration`);
            } catch (signInError) {
          ...
          console.error(`âŒ Error creating mentor ${mentorData.name}:`, error)
      ...
      if (auth.currentUser?.email !== adminEmail && adminEmail) {
        try {
          await signInWithEmailAndPassword(auth, adminEmail, adminPassword)
          console.log("âœ“ Signed back in as admin (final step)");
        } catch (error) {
          console.error("âŒ Error signing back in as admin:", error);
        }
      }
                    ...
                    <div className="space-y-6">
                      <div className="space-y-3">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => {
                            setNewMentor({ ...newMentor, role: "mentor" });
                          }}
                        >
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 my-4">
                    ...
                    {isBulkMode ? (
                      <div className="space-y-1 text-sm text-blue-800">
                        <p>â€¢ {bulkMentors.length} mentor accounts will be created</p>
                        <p>â€¢ Sequential mentor IDs will be assigned</p>
                        <p>â€¢ You will remain signed in as admin</p>
                      </div>
                    ) : (
                      <div className="space-y-1 text-sm text-blue-800">
                        <p>â€¢ Name: {newMentor.name}</p>
                        <p>â€¢ Email: {newMentor.email}</p>
                        <p>â€¢ Role: {newMentor.role}</p>
                        {newMentor.role === "mentor" && <p>â€¢ Mentor ID will be auto-assigned</p>}
                      </div>
                    )}
                  </div>
